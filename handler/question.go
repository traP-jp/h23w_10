...
type GetQuestionsByLimitAndOffsetResponce struct {
	ID        string       `json:"id"`
	UserID    string       `json:"user_id"`
	Title     string       `json:"title"`
	Content   string       `json:"content"`
...
func (h Handler) GetQuestionsByLimitAndOffset(c echo.Context, limit int, offset int) error {
	questions, err := h.qrepo.FindByLimitAndOffset(limit, offset)
	if err != nil {
		return err
	}
	response := make([]GetAllQuestionsResponse, len(questions))
